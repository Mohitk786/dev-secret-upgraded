...
export const downloadPrivateKey = (privateKey: string) => {
  const blob = new Blob([privateKey], { type: "text/plain" }); // Blob stands for Binary Large Object — it’s a way to represent raw data as a file-like object in the browser. it will create a virtual file in the browser  memory
...
export const encryptData = async (data: string, publicKeyBase64: string) => {
  const binaryDer = Uint8Array.from(atob(publicKeyBase64), (char) =>
    char.charCodeAt(0)
  );
...
export const decryptData = async (
  ...
  const binaryDer = Uint8Array.from(atob(privateKeyBase64), (char) =>
    char.charCodeAt(0)
  );
